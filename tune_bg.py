...
def run():
    ...
    generator.load_state_dict(torch.load('/home/pzq/pose_warp/trained_model/PNG_base2.pkl'))
    discriminator = Discriminator(128, 64).cuda()
    discriminator.load_state_dict(torch.load('/home/pzq/pose_warp/trained_model/PND_base2.pkl'))
        ...
        for batch_id in range(batch_num):
            ...
            for i in range(batch_img_num):
                ...
                if epo % interval != 0:
                    ...
                    for j in range(instance_num):
                        batch_gt[instance_num * i + j, :, :, :] = tgt_img * tgt_mask
                        batch_in[instance_num * i + j, 3, :, :] = tgt_pose
                        #batch_dis[instance_num * i + j, 3, :, :] = tgt_pose
                        batch_in[instance_num * i + j, :3, :, :] = warped_imgs[index, j, :, :, :] * tgt_mask
                        batch_mask[instance_num * i + j, :, :] = tgt_mask
            ...
            batch_mask = torch.tensor(batch_mask).float().cuda()
            ...
            if epo % interval == 0:
                gt_loss = mask_loss(out, batch_gt, batch_mask, batch_mask)
            else:
