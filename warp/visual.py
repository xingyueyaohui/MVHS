...
"""
This is pose information format
        img_info['img_id'] = img_name
        img_info['joints'] = joints
        img_info['joints3d'] = joints3d
        img_info['verts'] = verts
        img_info['cams'] = cams
"""
"""
...
def visualize_key_points(pose_path, img_dir, out_dir):
    ...
    for i in range(100, 120):
        person = info[i]
        img_name = person['img_id']
        img = cv2.imread(img_dir + img_name)
        joints = np.array(person['joints'][0])
        ...
        mask = np.where(new_img[:, :, 0] > 0, 1, 0)

        for c in range(3):
            new_img[:, :, c] = new_img[:, :, c] * mask + img[:, :, c] * (1 - mask)

        cv2.imwrite(out_dir + 'key_point' + img_name, new_img.astype(np.uint8))
...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    """
    if radius is None:
        radius = max(4, (np.mean(input_image.shape[:2]) * 0.01).astype(np.int32))
    """
...
if __name__ == '__main__':
    visualize_key_points('../data/pose/info.json', '../data/img/', '../data/vis_data/')
