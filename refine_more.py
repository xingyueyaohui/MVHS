"""
This is the file for second stage refinement
Structure similar to original_gan.py
"""
...
def run():
    ...
    generator.load_state_dict(torch.load('/home/pzq/pose_warp/trained_model/PNG_sk.pkl'))
    for param in generator.parameters():
        param.requires_grad = False
    refiner = Refiner().cuda()
    refiner.load_state_dict(torch.load('/home/pzq/pose_warp/pretrain/REF_s11e19.pkl'))
    discriminator = Discriminator(128, 64).cuda()
    #discriminator.load_state_dict(torch.load('/home/pzq/pose_warp/trained_model/D_sk.pkl'))

    criterion = nn.L1Loss()
    advLoss = nn.BCELoss()
    G_optimizer = optim.Adam(refiner.parameters(), lr=lr_generator, betas = (0.5, 0.999))
    ...
    for epo in range(epoch):
        ...
        for batch_id in range(batch_num):
            ...
            out = refiner(generator(batch_in))
            ...
            #print('Batch [{i}] in epoch [{e}], distance is [{distance}]'.format(
            #        i=batch_id, e=epo, distance=gt_loss))
            total_G_loss += G_loss
            total_D_loss += D_loss
            total_loss += gt_loss
        ...
        if (epo + 1) % 5 == 0:
            torch.save(refiner.state_dict(), '/home/pzq/pose_warp/pretrain/REFGAN_s{s}e{epo}.pkl'.format(s=session,epo=epo))
            torch.save(discriminator.state_dict(), '/home/pzq/pose_warp/pretrain/DREFGAN_s{s}e{epo}.pkl'.format(s=session,epo=epo))
...
def out_model():
    ...
    print("Loading Done. Start perquisite job.")
    mapping = pre.construct_mapping(data_json_path)
    src_imgs = pre.read_color(data_json_path, width, height, src_dir, start=0, end=1000) / 255
    print('Read in src images done')
    pose_map = pre.read_gray(data_json_path, width, height, pose_map_dir, start=0, end=1000) / 255
    tgt_masks = pre.read_gray(data_json_path, width, height, tgt_mask_dir, start=0, end=1000) / 255
    print('Gray images reading done')
    warped_imgs = pre.read_morphed_img_and_group(warp_dir, data_json_path, match_path, width, height, num=2, start=0, end=1000) / 255
    print('Warped images reading done')
